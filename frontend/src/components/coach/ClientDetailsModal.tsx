...
export function ClientDetailsModal({ client, isOpen, onClose, onUpdate }: ClientDetailsModalProps) {
  ...
  const loadClientData = async () => {
    try {
      ...
      const [notesResponse, insightsResponse] = await Promise.all([
        fetch(`${apiUrl}/api/v1/coach/clients/${client.id}/notes`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
        }),
        fetch(`${apiUrl}/api/v1/coach/clients/${client.id}/insights`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
  ...
  const saveClientNotes = async () => {
    try {
      ...
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
      const response = await fetch(`${apiUrl}/api/v1/coach/clients/${client.id}/notes`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
