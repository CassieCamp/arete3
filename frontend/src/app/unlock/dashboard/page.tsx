...
import { ClientManagement } from '@/components/unlock/ClientManagement';
import { ResourceManagement } from '@/components/unlock/ResourceManagement';
import { TemplateManagement } from '@/components/unlock/TemplateManagement';
  ...
  const loadCoachData = async () => {
    try {
      ...
      const [clientsResponse, resourcesResponse] = await Promise.all([
        fetch(`${apiUrl}/api/v1/unlock/clients`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
        }),
        fetch(`${apiUrl}/api/v1/unlock/resources`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
